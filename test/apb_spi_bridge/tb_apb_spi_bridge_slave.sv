...
//   assign miso = mosi;
    ...
    logic [15:0] slave_tx_data = 16'h5A7A;
    ...
    always @(negedge cs_n) begin
        for (integer i = 15; i >= 0; i--) begin
            miso <= slave_tx_data[i];

            @(posedge sclk);
            slave_rx_data[i] = mosi;
    ...
    initial begin
        ...
        if (PRDATA[15:0] == 16'h5A7A)
            $display("SUCCESS: Received Data Matched (0x5A7A)");
        else
            $display("FAILURE: Expected 0x5A7A, got 0x%h", PRDATA);
        
        if (slave_rx_data == 16'hABCD)
            $display("SUCCESS: Dummy Received Data Matched (0xABCD)");
        else
            $display("FAILURE: Dummy Expected 0xABCD, got 0x%h", slave_rx_data);
